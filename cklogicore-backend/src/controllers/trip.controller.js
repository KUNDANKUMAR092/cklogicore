...
const catchAsync = (fn) => (req, res, next) => {
  Promise.resolve(fn(req, res, next)).catch(next); // Ensure 'next' is passed here
};

// CREATE
// Signature me (req, res, next) zaroor rakhein
export const createTrip = catchAsync(async (req, res, next) => { // 'next' yahan bhi add karein
...
export const getTrips = catchAsync(async (req, res) => {
  ...
  const [totalRecords, trips, summary] = await Promise.all([
  ...
  res.json({
    success: true,
    pagination: { totalRecords, currentPage: Number(page), totalPages: Math.ceil(totalRecords / limit) },
    summary: summary[0] || { totalProfit: 0, totalCompanyPay: 0, totalVehiclePay: 0, totalSupplierPay: 0 },
    data: trips
  });
});
...
export const addChallans = catchAsync(async (req, res) => {
  ...
  if (!trip) {
    // If trip not found, cleanup uploaded files from disk immediately
    req.files.forEach(f => {
