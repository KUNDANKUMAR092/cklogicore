...
export default function Dashboard() {
  ...
  // 3. API DATA FETCHING
  const { data: response, error, isLoading, isFetching } = useGetDashboardStatsQuery(filters, { refetchOnMountOrArgChange: true });
  ...
  // 4. DEBOUNCE EFFECT (Focus maintainer)
  useEffect(() => {
  ...
  const handleExport = async () => {
    ...
    } catch (err) { toast.error("Export failed. Please check login."); }
  };

  if (isLoading || isFetching) return (
    <div className="flex flex-col items-center justify-center p-10 min-h-screen">
      <Loader />
      <p className="text-gray-500 mt-2 text-sm animate-pulse">Analysing Business...</p>
    </div>
  );

  if (error) return <div className="p-20 text-center text-red-500 font-bold">Error loading dashboard.</div>;
    ...
    <div className="md:p-6 min-h-screen font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div className="flex items-center gap-3 bg-white p-2 rounded-2xl shadow-sm border border-gray-100">
          <div className="flex items-center gap-2 px-3 border-r">
            ...
            <select value={exportTarget} onChange={(e) => setExportTarget(e.target.value)} className="text-xs font-bold text-gray-600 outline-none bg-transparent cursor-pointer">
          ...
          <button onClick={handleExport} className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white rounded-xl text-xs font-bold hover:bg-blue-700 transition-all shadow-lg shadow-blue-200">
      ...
      <div className="bg-white p-4 md:p-5 rounded-[1.5rem] md:rounded-[2.5rem] shadow-sm border border-gray-100 mb-8 flex flex-col gap-4 w-full">
        <div className="flex flex-col md:flex-row items-center gap-3 md:gap-4 w-full">
          <div className="flex items-center gap-2 w-full md:flex-1">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3 bg-gray-50 rounded-2xl text-sm outline-none focus:ring-2 ring-blue-500 border border-transparent"
                value={localSearchTerm}
                onChange={(e) => setLocalSearchTerm(e.target.value)}
              />
            </div>
            <button onClick={resetFilters} className="flex md:hidden items-center gap-2 px-3 py-3.5 text-gray-500 bg-gray-50 rounded-2xl border border-gray-100 shrink-0">
              <FaSyncAlt size={14} />
            </button>
          </div>

          {/* DATE FILTER (Strict: Start < End) */}
          <div className="flex items-center justify-between gap-2 bg-gray-50 p-2.5 rounded-2xl border border-gray-100 w-full md:w-auto md:min-w-[320px]">
            ...
            <div className="flex items-center gap-2 flex-1">
              ...
              <input
                type="date"
                className="bg-transparent text-[11px] sm:text-xs outline-none w-full cursor-pointer"
                value={filters.startDate}
                // Start date aaj se aage nahi ja sakti
                max={today} 
                onChange={(e) => {
                  const val = e.target.value;
                  // 🟢 Agar user ne Start Date "Today" select ki hai, toh End Date ko clear kar do
                  if (val === today) {
            ...
            <div className="flex-1">
              <input
                ...
                // 🟢 LOGIC: 
                // 1. Agar Start Date nahi hai -> Disable
                // 2. Agar Start Date "Today" hai -> Disable (kyunki future date allowed nahi h)
                disabled={filters.startDate === today}
                // End date hamesha Start date se kam se kam 1 din badi hogi
                min={filters.startDate ? getNextDay(filters.startDate) : ""}
                max={today}
          ...
          <button onClick={resetFilters} className="hidden md:flex p-3.5 text-gray-400 hover:text-blue-500 transition-colors bg-gray-50 rounded-2xl border border-transparent hover:border-gray-200 cursor-pointer">
            <FaSyncAlt size={14} className="mr-2" />
            <span className="text-xs font-bold">Reset Filter</span>
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatCard title="Total Profit" value={`₹${rawData.overview?.totalProfit?.toLocaleString() || 0}`} color="text-green-600" icon="📈" />
        <StatCard title="Total Revenue" value={`₹${rawData.overview?.totalRevenue?.toLocaleString() || 0}`} color="text-blue-600" icon="💰" />
        <StatCard title="Total Trips" value={rawData.overview?.totalTrips || 0} color="text-indigo-600" icon="🚚" />
        <StatCard title="Total Weight" value={`${rawData.overview?.totalWeight || 0} MT`} color="text-orange-600" icon="⚖️" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={formattedMetricsData}>
              ...
              <Bar dataKey="value" radius={[10, 10, 0, 0]}>
                {formattedMetricsData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </ChartContainer>
        ...
        <ChartContainer title="Operational Mix Distribution">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={formattedMetricsData} dataKey="value" nameKey="name" innerRadius={70} outerRadius={100} paddingAngle={8}>
                {formattedMetricsData.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} cornerRadius={10} stroke="none" />)}
              </Pie>
              <Tooltip />
...
const Dropdown = ({ icon, label, value, options, name, onChange, displayKey = "name" }) => (
  <div className="relative">
    ...
    <select className="w-full pl-11 pr-4 py-3.5 bg-gray-50 border-none rounded-2xl text-sm outline-none focus:ring-2 ring-blue-500 appearance-none cursor-pointer font-medium" value={value} onChange={(e) => onChange(name, e.target.value)}>
