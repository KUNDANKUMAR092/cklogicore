...
export default function Dashboard() {
  ...
  // 3. API DATA FETCHING
  const { data: response, error, isLoading, isFetching } = useGetDashboardStatsQuery( 
    filters,  
...
const Dropdown = ({ icon, label, value, options, name, onChange, displayKey = "name" }) => (
  <div className="relative">
    ...
    <select className="w-full pl-11 pr-4 py-3.5 bg-gray-50 border-none rounded-2xl text-sm outline-none focus:ring-2 ring-blue-500 appearance-none cursor-pointer" value={value} onChange={(e) => onChange(name, e.target.value)}>
