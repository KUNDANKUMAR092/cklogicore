...
const CrudForm = ({ form, setForm, fields, onRemoveChallan }) => {
  ...
  // 🟢 Edit/Update Logic: Yeh sirf tab trigger hoga jab form load hoga
  useEffect(() => {
    if (form?.financials) {
      const f = form.financials;
      // Agar kisi bhi field mein 0 se bada data hai, to checkbox ON dikhega
      const hasData = (
        Number(f.companyAdvance || 0) > 0 || 
        Number(f.companyDiesel || 0) > 0 || 
        Number(f.companyTollCost || 0) > 0 || 
        Number(f.companyDriverExpense || 0) > 0 || 
        Number(f.companyOtherExpense || 0) > 0
      );
      setShowCompanyFinancials(hasData);
    }
  }, [form?._id]); // Jab trip change ho ya naya form khule

  /* ===============================
      CALCULATION HELPER (Safe)
  =============================== */
  const updateTripCalculations = (currentForm) => {
    // Agar trip related data nahi hai (jaise Supplier form), to return kar jao
  ...
  const handleToggleFinancials = (e) => {
    ...
    if (!isChecked) {
      // Jab user uncheck kare, to values 0 karke calculation reset karein
      setForm((prev) => {
  ...
  const handleChange = (e, validationType) => {
    ...
    let finalValue = type === "file" 
      ? [...(Array.isArray(form.challans) ? form.challans : []), ...Array.from(files)]
      : value;
  ...
  return (
    <div className="flex flex-col space-y-6 max-h-[calc(100vh-250px)] overflow-y-auto px-1 pb-4 scrollbar-hide">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
        {fields.map((field) => {
          if (field.hidden) return null;

          // 🟢 SAFE RENDERING: Only hide company financial fields if checkbox is off
            ...
            <div key={field.name} className="flex flex-col space-y-1.5">
              ...
              {field.type === "select" ? (
                ...
                <input
                  ...
                  name={field.name}
                  value={field.type !== "file" ? getValue(field) : undefined}
                  onChange={(e) => handleChange(e, field.validationType)}
                  disabled={isDisabled}
                  placeholder={`Enter ${field.label}...`}
                  className={`w-full px-4 py-3 border rounded-2xl text-sm font-medium outline-none transition-all
                    ${error ? "border-red-500" : "border-gray-200 focus:border-blue-400"}
                    ${isDisabled ? "bg-gray-50 opacity-70" : "bg-white"}`}
                />
              )}
            </div>
          );

          // 🟢 CHECKBOX INJECTOR: Supplier section ke khatam hote hi dikhega
          if (field.name === "financials.supplierOtherExpense") {
            return (
              <React.Fragment key="toggle-section">
                {fieldBody}
                <div className="col-span-1 md:col-span-2 flex justify-end items-center my-2">
                  <div className="flex items-center space-x-2 bg-blue-50/60 px-3 py-2 rounded-xl border border-blue-100 shadow-sm">
                    <input 
                      type="checkbox" 
                      id="updateCompCheck" 
                    ...
                    />
                    <label htmlFor="updateCompCheck" className="text-[10px] font-black text-blue-700 cursor-pointer uppercase tracking-tighter">
                      Add/Edit Company Expenses?
                    </label>
                  </div>
                </div>
              </React.Fragment>
      ...
      {/* Challans Thumbnail Rendering */}
      {form.challans && form.challans.length > 0 && (
        <div className="mt-4 p-4 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200">
          <p className="text-[10px] font-black text-gray-400 uppercase mb-3">Documents ({form.challans.length}/4)</p>
            ...
            {form.challans.map((file, index) => (
              <ChallanThumbnail
                ...
                onDelete={file._id ? (tId, cId) => onRemoveChallan(tId, cId, setForm) : () => {
                   const updated = form.challans.filter((_, i) => i !== index);
